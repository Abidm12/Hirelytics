...
# ✅ Once logged in, proceed with insights
...
if selected_section == "Placement Prediction":
    ...
    if predict_button:
        if df is not None and {'CGPA', 'Internship', 'Skills', 'Package'}.issubset(df.columns):
            ...
            st.info("⚠️ Prediction model requires placement data. Without it, only manual resume suggestions are available.")
...
# --- Resume Analyzer ---
    ...
    if resume_file is not None:
        ...
        if df is not None and "Skills" in df.columns and "Internship" in df.columns:
            ...
            st.info("⚠️ Resume scanned successfully, but placement dataset is missing. Skill gap analysis unavailable.")
